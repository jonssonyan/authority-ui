...
<script>
    export default {
        ...
        methods: {
            async getRoleList() {
                let {data: res} = await this.$http.post(`/api/role/selectPage`, this.queryInfo);
            ...
            async stateChange(info) {
                let {data: res} = await this.$http.post(`/api/role/saveOrUpdate`, info);
                if (res.code !== 1) {
            ...
            addRole() {
                this.$refs.addRoleRef.validate(async valid => {
                    if (!valid) return;
                    // 发起添加角色的网络请求
                    const {data: res} = await this.$http.post('/api/role/saveOrUpdate', this.addRoleForm);
            ...
            async showEditDialog(role) {
                const {data: res} = await this.$http.post(`/api/role/getById`, role);
                if (res.code !== 1) return this.$message.error("查询角色失败");
            ...
            editRoleInfo() {
                this.$refs.addRoleRef.validate(async valid => {
                    if (!valid) return;
                    // 发起修改角色的网络请求
                    const {data: res} = await this.$http.post(`/api/role/saveOrUpdate`, this.editRoleForm);
            ...
            async removeById(role) {
                ...
                await this.$confirm('此操作将永久删除该角色, 是否继续?', '提示', {
                    ...
                    // 删除角色
                    const {data: res} = await this.$http.post(`/api/role/removeById`, role);
                    if (res.code !== 1) return this.$message.error("删除角色失败");
